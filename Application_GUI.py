...
class MyWindow(QMainWindow):
    ...
    def setup_window(self):
        ...
        logo_path = os.path.join(base_path, "app_logo.png")
    ...
    # Create a QTextEdit widget to display formatted information
        ...
        content1 = self.read_and_format_file(information1_path)
        content2 = self.read_and_format_file(information2_path)
    ...
    def document_generation_feedback(self):
        if hasattr(sys, '_MEIPASS'):
            ...
            base_directory = os.path.dirname(os.path.abspath(__file__))  # Path to the current script directory
        ...
        name_list = ["Inderjit_Singh", "Michal_Fabian", "Jose_Ramirez", 
                     ...
                     "Ulises_Romero", "Grace_Bolton", "Colm_Crossey", 
                     "Joshua_Johnson", "Yadira_Lopez"]
        
        image = ["Andres.jpg", "ssg_logo.jpg", "Fortnite.jpg", "Eagle.jpg", "chun.jpg", 
                 "Corey.jpg", "Smash.jpg", "Crazy_Frog.jpg", "pizza.jpg", "Moist.jpg", 
                 "Patrick.jpg", "oiia.jpg", "ssg_logo.jpg", "magdonal.jpg", "dog.jpg"]
        audio = ["Notification_Sound.mp3", "Notification_Sound.mp3", "Fortnite.mp3", "Eagle.mp3", "Notification_Sound.mp3", 
                 "Notification_Sound.mp3", "Smash.mp3", "Crazy_Frog.mp3", "Notification_Sound.mp3", "Moist.mp3", 
                 "Patrick.mp3", "oiia.mp3", "AUUGHHH.mp3", "Notification_Sound.mp3", "Notification_Sound.mp3"]
        ...
        if name in name_list:
            ...
            if name == "Andres_Aranda":
                ...
                meme = random.randint(2, 15) - 1
            # Selects the meme
            image_path = os.path.join(base_directory, f"memes/{image[meme]}")
            ...
            audio_path = os.path.join(base_directory, f"memes/{audio[meme]}")
            player = QtMultimedia.QMediaPlayer()
            audio_file = QtCore.QUrl.fromLocalFile(audio_path)
            content = QtMultimedia.QMediaContent(audio_file)
            player.setMedia(content)
            
        else:
            # Default option for image and audio
            image_path = os.path.join(base_directory,"ssg_logo.jpg")
            image_label = QLabel(dialog)
            image_label.setPixmap(QtGui.QPixmap(image_path))
            image_label.setScaledContents(True)
            layout.addWidget(image_label)
            
            audio_path = os.path.join(base_directory,"Notification_Sound.mp3")
            player = QtMultimedia.QMediaPlayer()
            audio_file = QtCore.QUrl.fromLocalFile(audio_path)
            content = QtMultimedia.QMediaContent(audio_file)
            player.setMedia(content)
        
        message_label = QLabel("Your document has been generated. Please go to Rest Period Acknowledgement Form folder in SSG Customer Access and sign the document.", dialog)
        ...
        try:
            ...
            self.userAlertWindow('Failed to create docx!', 'Critical')
        
        # This turns the doc file to pdf and had to capture the output since convert needs a console and as a app I don't want a console
        ...
        try:
            ...
            self.userAlertWindow("Failed to move pdf to onedrive!", "Critical")
    ...
    def on_transfer_clicked(self):
        dialog = QDialog()
        dialog.setWindowTitle("Enter Password to Transfer Files")
        dialog.setGeometry(700, 400, 400, 300)
        
        layout = QVBoxLayout(dialog)
        
        password_input = QLineEdit()
        password_input.setPlaceholderText("Please enter the password")
        password_input.setEchoMode(QLineEdit.Password)  # Hide input text when user is entering password
        layout.addWidget(password_input)
        
        submit_button = QPushButton("Submit", dialog)
        layout.addWidget(submit_button)
        
        # Connect the button to accept the dialog
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            if password == "ssgllc08":
                try:
                    success = self.file_manipulation.transfer_all_file_button()
                    if success:
                        QMessageBox.information(dialog, "Confirmation", "Files have been moved.")
                    else:
                        self.userAlertWindow('Failed to transfer all files', 'Critical')
                except:
                    self.userAlertWindow('Failed to transfer all files', 'Critical')
